...
export const Badge: React.FC<BadgeProps> = ({
  ...
  const variantStyles = {
    default: 'bg-slate-100 text-slate-700 dark:bg-white/[0.06] dark:text-zinc-300 border-slate-200 dark:border-white/[0.08]',
    ...
    gray: 'bg-zinc-100 text-zinc-600 dark:bg-zinc-800/60 dark:text-zinc-400 border-zinc-200 dark:border-white/[0.08]',
  };
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-md border tracking-wide uppercase font-semibold ${variantStyles[variant]} ${sizeStyles[size]} ${className}`}
    >
      {children}
    </span>
  );
